...
def test_zero_delta_returns_single_point():
    traj = trapezoidal_profile(
        p0=45.0,
        v0=0.0,
        p_target=45.005,  # < 0.01° threshold
        v_max=V_MAX,
        a_max=A_MAX,
        tick_dt=TICK,
    )
...
def test_wrap_around_path_short_way():
    ...
    assert traj.total_duration < 6.0, (
        f"total_duration={traj.total_duration} suggests long path was chosen"
    )
...
def test_trapezoid_t_offset_sample_during_hold():
    ...
    for t_probe in (0.0, 0.1, 0.25, 0.49):
        ...
        assert abs(s.pos - 25.0) < 1e-6, f"at t={t_probe} pos should be 25, got {s.pos}"
...
def test_2d_scurve_pure_az_matches_1d():
    ...
    assert (
        abs(
            _max_abs([p.vel for p in traj_az_2d.points])
            - _max_abs([p.vel for p in traj_1d.points])
        )
        < 1e-3
    )
