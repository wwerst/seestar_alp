...
Usage examples:
    ...
    uv run python scripts/tune_vc.py \\
        ...
        --control feedforward --kp-pos 0.5 --tol 0.3 --alt 10.0
...
def _print_latency_summary(cli: InstrumentedAlpacaClient) -> None:
    ...
    print()
    print(
        "HTTP command latency (ms, RPC round-trip through the Alpaca action endpoint)"
    )
    print(
        f"  {'method':<24}  {'n':>5}  {'mean':>6}  {'p50':>6}  {'p90':>6}  {'p99':>6}  {'max':>6}"
    )
...
def run_setpoints(cli: InstrumentedAlpacaClient, args) -> int:
    ...
    print("=" * 78)
    print("Velocity-controller tuning — setpoint mode (FF+FB mover)")
    print(
        f"  control={args.control}  kp_pos={args.kp_pos}  v_corr_max={args.v_corr_max}"
    )
    print(
        f"  max_rate={args.max_rate}°/s  a_max={args.a_max}  j_max={args.j_max}  "
        f"profile={args.profile}  loop_dt={args.loop_dt}s"
    )
    ...
    try:
        ...
        if az_limits is not None:
            ...
            print(
                f"Initial arrived: measured_az={cur_az:+.3f}°  "
        ...
        for i, target in enumerate(setpoints, start=1):
            ...
            if az_tracker is not None:
                try:
                    ...
                    print(
                        f"  (warning: CumulativeAzTracker.save failed: {e})",
                        file=sys.stderr,
                    )
            logger.mark_event(
                "setpoint_done",
        ...
        if az_tracker is not None:
            try:
                ...
                print(
                    f"  (warning: CumulativeAzTracker.save on exit failed: {e})",
                    file=sys.stderr,
                )
        logger.stop()
    ...
    print("=" * 78)
    header = (
        f"  {'step':>4}  {'target':>8}  {'residual':>9}  {'iter':>4}  "
...
def run_step_response(cli: InstrumentedAlpacaClient, args) -> int:
    ...
    if dur < _MIN_DUR_S:
        print(
            f"ERROR: step-dur must be >= {_MIN_DUR_S} (firmware floor)", file=sys.stderr
        )
        return 2
    ...
    print(
        f"Post-goto: mount idle reached after {idle_elapsed:.2f}s "
    ...
    for i, speed in enumerate(speeds, start=1):
        ...
        if abs(_wrap_pm180(cur_az - start_az)) > 40.0:
            print(
                f"[{i}/{len(speeds)}] re-centering to {start_az:+.1f}° before next burst..."
            )
            init_ra, init_dec = issue_slew(cli, start_az, args.alt, loc)
            wait_until_near_target(
                cli,
        ...
        print(
            f"[{i}/{len(speeds)}] step-response: speed={speed} angle={angle} dur={dur}s"
        )
    ...
    print("=" * 78)
    print(
        f"  {'speed':>6}  {'angle':>5}  {'r_ss_exp':>10}  {'r_ss_fit':>10}  {'τ (s)':>7}  {'rmse':>6}"
    )
    for r in results:
        tau_s = f"{r['tau_s']:>7.3f}" if r["tau_s"] is not None else "    -  "
        rss_f = (
            f"{r['r_ss_fitted_degs']:>+9.3f}"
            if r["r_ss_fitted_degs"] is not None
            else "     -   "
        )
        rmse = (
            f"{r['fit_rmse_deg']:>5.3f}" if r["fit_rmse_deg"] is not None else "   -  "
        )
        print(
...
def run_diagonal(cli: InstrumentedAlpacaClient, args) -> int:
    ...
    print(
        f"  speed={speed}  dur_sec={dur}  home=(alt={home_alt:.1f}°, az={home_az:.1f}°)"
    )
    ...
    for i, angle in enumerate(angles, start=1):
        print(
            f"[{i}/{len(angles)}] recentering to (alt={home_alt:.1f}, az={home_az:.1f})..."
        )
        ...
        print(
            f"  expected (vector): Δaz={exp_d_az:+.3f}°  Δalt={exp_d_alt:+.3f}°  "
            f"|v|_nominal={nominal_motion:.3f}°"
        )
        print(
            f"  obs_dir={dir_obs:+.1f}° (cmd angle={angle:+.1f}°)  "
        ...
        results.append(
            {
                ...
                "exp_d_alt": exp_d_alt,
                "mag_obs": mag_obs,
                "mag_exp": nominal_motion,
                "dir_obs": dir_obs,
                "dir_err": _wrap_pm180(dir_obs - angle),
                "ratio_mag": ratio_mag,
            }
        )
        time.sleep(0.5)
...
def main() -> int:
    ...
    p.add_argument(
        "--mode",
        choices=["setpoints", "step_response", "diagonal"],
        default="setpoints",
    )
    ...
    p.add_argument(
        "--step-log", default=None, help="Path to a JSONL file to write raw samples to."
    )
    ...
    p.add_argument(
        "--diag-speed", type=int, default=500, help="Speed for each diagonal burst."
    )
    p.add_argument(
        "--diag-dur", type=int, default=5, help="Burst duration (≥ _MIN_DUR_S=5)."
    )
    p.add_argument(
        "--diag-home-alt",
        type=float,
    ...
    p.add_argument(
        "--diag-home-az", type=float, default=0.0, help="Home azimuth for recentering."
    )
    args = p.parse_args()
